...
function showOutSupList(){
    let list = [...new Set(allGoods.map(item=>item.supplier).filter(s=>s))];
...
function filterOutSupList(){
    ...
    let list = [...new Set(allGoods.map(item=>item.supplier).filter(s=>s))].filter(s => s.toLowerCase().includes(kw));
    let box = document.getElementById('outSupListBox');
    box.innerHTML = '';
    if(list.length === 0){
        box.innerHTML = '<div>无匹配数据</div>';
...
function showOutGoodsList(){
    let list = allGoods.filter(g => g.supplier === document.getElementById('outSupSearchInput').value);
...
function filterOutGoodsList(){
    ...
    let list = allGoods.filter(g => g.supplier === document.getElementById('outSupSearchInput').value && g.name.toLowerCase().includes(kw));
    let box = document.getElementById('outGoodsListBox');
    box.innerHTML = '';
    if(list.length === 0){
        box.innerHTML = '<div>无匹配数据</div>';
...
function selectOutGoods(goods){
    ...
    // 读取总库存
    let total = getTotalStockNum(document.getElementById('outSupSearchInput').value, goods.name);
...
function filterStockOut() {
    ...
    let filtered = allStockOut.filter(item => String(item[field]||'').toLowerCase().includes(kw));
    document.getElementById('outSearchCount').textContent = filtered.length;
    outCurrentPage = 1;
    renderOutPagination();
...
function outSortTable(field) {
    ...
    let filtered = allStockOut.filter(item => String(item[document.getElementById('outSearchField').value]||'').toLowerCase().includes(document.getElementById('outSearchKeyword').value.toLowerCase()));
    filtered.sort((a,b)=>{
...
function updateOutSortIcon() {
    ...
    let idx = Array.from(document.querySelectorAll('#stockOut .sortable')).findIndex(th=>th.onclick?.toString().includes(outSortField));
    if(idx>-1) document.querySelectorAll('.outSortIcon')[idx].innerText = outSortAsc?'↑':'↓';
}

// 渲染出库表格
function renderStockOut(list) {
    if(!list) list = allStockOut;
    let start = (outCurrentPage-1)*outPageSize;
    let pageData = list.slice(start, start+outPageSize);
...
function renderOutPagination() {
    let filtered = allStockOut.filter(item => String(item[document.getElementById('outSearchField').value]||'').toLowerCase().includes(document.getElementById('outSearchKeyword').value.toLowerCase()));
    outTotalPages = Math.ceil(filtered.length/outPageSize)||1;
...
function exportStockOutExcel(){
    ...
    let expData = allStockOut.map(item=>[
        item.supplier||"",
        item.goodsName||"",
        item.spec||"",
        item.settleType||"",
