...
async function saveInvoiceBackRecord() {
    ...
    try {
        ...
        // 5. ✅ 刷新入库列表（调用 in.js 中的 loadStockIn）
...
async function autoWriteOffInvoice(supplier, invoiceAmount, invoiceNo) {
    ...
    // 2. 过滤该供应商的未开票线下入库记录（按入库日期从早到晚排序）
    ...
    console.log(`找到 ${inRecords.length} 条未开票入库记录`, inRecords);

    if (inRecords.length === 0) {
        console.log(`供应商 ${supplier} 没有未开票的入库记录`);
        showMsg(`注意：供应商 ${supplier} 没有未开票的入库记录可核销`);
        return;
    }
    ...
    for (let record of inRecords) {
        ...
        console.log(`处理入库记录 ID:${record.id}，金额:${recordTotal}，剩余发票:${remainingAmount}`);
        
        if (remainingAmount >= recordTotal) {
            // 完全核销该条记录
            remainingAmount -= recordTotal;
            updatedIds.push({ id: record.id, status: '已开票' });
            console.log(`✅ 完全核销记录 ID:${record.id}`);
        } else {
            // 部分核销：标记为"未开票"（但记录已处理）
            updatedIds.push({ id: record.id, status: '未开票' });
            remainingAmount = 0;
            console.log(`⚠️ 部分核销记录 ID:${record.id}，剩余发票已用完`);
        }
    }
    ...
    for (let item of updatedIds) {
        ...
        try {
            const result = await fetch(`${SUPABASE_URL}/rest/v1/stock_in?id=eq.${item.id}`, {
                method: 'PATCH',
                headers: headers,
            ...
            if (result.ok) {
                updateCount++;
                console.log(`✅ 更新入库记录 ${item.id} 状态为 ${item.status}`);
            } else {
                console.error(`❌ 更新入库记录 ${item.id} 失败`, await result.text());
            }
        } catch (e) {
    ...
    if (remainingAmount > 0) {
        console.log(`供应商 ${supplier} 发票余额 ${remainingAmount.toFixed(2)} 无更多未开票入库记录可核销`);
        showMsg(`注意：供应商 ${supplier} 发票余额 ${remainingAmount.toFixed(2)} 无更多未开票入库记录可核销（已核销 ${updateCount} 条）`);
    } else {
        showMsg(`✅ 核销完成：共更新 ${updateCount} 条入库记录为"${updatedIds.length > 0 ? updatedIds[updatedIds.length-1].status : '已开票'}"`);
    }
}
...
function searchStockInCheck() {
    ...
    list = list.map(row => {
        ...
        if (channel === '线上') {
            ...
            // 线下供应商
            taxRateDisplay = (taxRate > 0 ? taxRate + '%' : '0%');
            inPriceDisplay = formatMoney(inPrice);
            ...
            remainAmount = formatMoney(backTotal - totalAmount);
        }
        ...
        return {
            ...
            noTaxPrice: formatMoney(noTaxPrice),
            noTaxTotal: formatMoney(noTaxTotal),
            taxTotal: formatMoney(taxTotal),
            isPay: isPay,
            remainAmount: remainAmount,
            record_date: recordDate  // ✅ 保留录入日期
        };
    });
        ...
        list.forEach(row => {
            ...
            if (!groupMap[key]) {
                groupMap[key] = {
                    ...
                    isPay: row.isPay,
                    remainAmount: 0,
                    record_date: row.record_date || '',  // ✅ 保留录入日期
                    count: 0
                };
            }
            const g = groupMap[key];
            g.in_num += Number(row.in_num);
            g.totalAmount += parseFloat(row.totalAmount.replace('￥', ''));
            g.noTaxTotal += parseFloat(row.noTaxTotal.replace('￥', ''));
            g.taxTotal += parseFloat(row.taxTotal.replace('￥', ''));
            const remainVal = typeof row.remainAmount === 'string' ? parseFloat(row.remainAmount.replace('￥', '')) : Number(row.remainAmount);
            g.remainAmount += isNaN(remainVal) ? 0 : remainVal;
            g.count++;
            if (g.count === 1) {
                g.in_price_display = row.in_price_display;
                g.record_date = row.record_date || '';  // ✅ 取第一条的日期
            }
        });
        list = Object.values(groupMap).map(g => ({
            ...g,
            totalAmount: formatMoney(g.totalAmount),
            noTaxTotal: formatMoney(g.noTaxTotal),
            taxTotal: formatMoney(g.taxTotal),
            remainAmount: formatMoney(g.remainAmount)
        }));
    }
